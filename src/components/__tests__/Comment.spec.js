...
describe('Comment.vue', () => {
  test('sets router-link to prop using comment.by', () => {
    ...
    expect(wrapper.findAll('.comment__by')).toHaveLength(1);
  ...
  test('initially renders open toggle if comment has kids', () => {
    ...
    expect(wrapper.find('.comment__toggle').text()).toContain('[-]');
  });
  ...
  test('renders closed toggle after click if comment has kids', async () => {
    ...
    expect(wrapper.find('.comment__toggle').text()).toContain('[+] 2 replies collapsed');
  });

  test('does not render toggle if comment has no kids', () => {
    const wrapper = createWrapper();

    expect(wrapper.find('.comment__toggle').exists()).toEqual(false);
  });
